...
NxScreen::NxScreen( int MonitorID, bool Fullscreen, unsigned int Width, unsigned int Height ) : mRenderWindow(0), mOpacity(1.0f)
{
	...
	{ 
		...
		mTextureUnitState->setTextureName( "RTT_Texture_100", TextureType::TEX_TYPE_2D );
...
NxScreen::~NxScreen()
{
	...
	{
		delete mPatch;
		LogMsg("Destroying External Window : " + mRenderWindow->getName() );
		mRenderWindow->setActive( false );
