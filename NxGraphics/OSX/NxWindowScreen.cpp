...
#include "NxGraphics_OgreConversions.h"
...
NxScreen::NxScreen( int MonitorID, bool Fullscreen, unsigned int Width, unsigned int Height ) : mRenderWindow(0), mOpacity(1.0f)
{
	...
	Log( "Creating NxOutput for " +  std::string( mMonitorInfo->GetDeviceName() )  );
	...
	{ 
		...
		options["externalWindowHandle"] = Ogre::StringConverter::toString( reinterpret_cast<uintptr_t>( mWindowHandle ) );
