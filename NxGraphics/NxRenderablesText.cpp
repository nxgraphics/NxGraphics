...
NxBillboardText::NxBillboardText(const String &name, const String &caption, const String &fontName, Real charHeight, const ColourValue &color)
	: mpCam(NULL)
	, mpWin(NULL)
	, mpFont(NULL)
	, mName(name)
...
NxBillboardText::~NxBillboardText()
{
...
void NxBillboardText::setFontName(const String &fontName)
{
...
void NxBillboardText::setCaption(const String &caption)
{
	if (caption != mCaption)
	{
		mCaption = caption;
...
void NxBillboardText::setColor(const ColourValue &color)
{
	if (color != mColor)
	{
		mColor = color;
...
void NxBillboardText::setCharacterHeight(Real height)
{
...
void NxBillboardText::setSpaceWidth(Real width)
{
...
void NxBillboardText::setTextAlignment(const HorizontalAlignment& horizontalAlignment, const VerticalAlignment& verticalAlignment)
{
...
void NxBillboardText::setGlobalTranslation( Ogre::Vector3 trans )
{
	mGlobalTranslation = trans;
}

void NxBillboardText::setLocalTranslation( Ogre::Vector3 trans )
{
	mLocalTranslation = trans;
}

void NxBillboardText::showOnTop(bool show)
{
...
void NxBillboardText::_setupGeometry()
{
	assert(mpFont);
	...
	{
	case NxBillboardText::V_ABOVE:
		verticalOffset = mCharHeight;
		break;
	case NxBillboardText::V_CENTER:
		verticalOffset = 0.5*mCharHeight;
		break;
	case NxBillboardText::V_BELOW:
		verticalOffset = 0;
		break;
	}
	...
	for (i = mCaption.begin(); i != iend; ++i)
	{
		...
		//
		// Upper left
		if(mHorizontalAlignment == NxBillboardText::H_LEFT)
			*pPCBuff++ = left;
		else
			*pPCBuff++ = left - (len / 2);
		*pPCBuff++ = top;
...
void NxBillboardText::_updateColors(void)
{
	assert(mpFont);
	assert(!mpMaterial.isNull());
...
const Ogre::Quaternion& NxBillboardText::getWorldOrientation(void) const
{
	assert(mpCam);
...
void NxBillboardText::visitRenderables(Ogre::Renderable::Visitor* visitor, 
								   bool debugRenderables)
{
}

const Ogre::Vector3& NxBillboardText::getWorldPosition(void) const
{
	assert(mParentNode);
	return mParentNode->_getDerivedPosition();
}

void NxBillboardText::getWorldTransforms(Ogre::Matrix4 *xform) const 
{
...
void NxBillboardText::getRenderOperation(RenderOperation &op)
{
	if (this->isVisible())
	{
		if (mNeedUpdate)
...
void NxBillboardText::_notifyCurrentCamera(Camera *cam)
{
	mpCam = cam;
}

void NxBillboardText::_updateRenderQueue(RenderQueue* queue)
{
	if (this->isVisible())
	{
		if (mNeedUpdate)
		...
		queue->addRenderable(  this, RENDER_QUEUE_OVERLAY , RENDER_QUEUE_SKIES_LATE);
	}
}
